...
public class Retirement {
	...
	public double MonthlySavings() {
		double pmt = 0; // <-- this should be fixed to calculate the real p
		pmt = PMT(this.getdAnnualReturnWorking() / 12.0,
		this.getiYearsToWork() * 12.0, 0.0, this.TotalAmountToSave(), false);
		...
		double pv = 0;
		pv = PV(this.getdAnnualReturnRetired() / 12.0, this.getiYearsRetired()
		* 12.0, this.getdRequiredIncome() - this.getdMonthlySSI(), 0, false);
